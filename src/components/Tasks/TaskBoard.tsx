...
import { Plus, List, LayoutGrid, Calendar, Network, BarChart2, ArrowRight, PlusCircle } from "lucide-react";
...
import { WorkflowSettingsService, WorkflowState, WorkflowTransition, WorkflowTemplate } from "@/services/WorkflowSettings.service";
...
const TaskBoard: React.FC<TaskBoardProps> = ({
  ...
  // View state
  const [viewMode, setViewMode] = useState<'list' | 'kanban' | 'calendar' | 'dependencies' | 'workflow'>('list');
  ...
  useEffect(() => {
    if (kanbanSettings?.defaultView) {
      setViewMode(kanbanSettings.defaultView as 'list' | 'kanban' | 'calendar' | 'dependencies' | 'workflow');
    }
  }, [kanbanSettings]);
  ...
  const { data: filteredTasks = [] } = useQuery({
    queryKey: ['filtered-tasks', debouncedSearchQuery, filters.status, filters.priority, currentSort],
    queryFn: async () => {
  ...
  const handleViewModeChange = (mode: 'list' | 'kanban' | 'calendar' | 'dependencies' | 'workflow') => {
    setViewMode(mode);
    updateSettingsMutation.mutate({ defaultView: mode });
  };

  // Handle workflow settings save
  const handleSaveWorkflow = useCallback(async (states: WorkflowState[], transitions: WorkflowTransition[]) => {
              ...
              {[
                ...
                <Tooltip key={item.mode}>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <item.icon className={`h-4 w-4 mr-1 ${viewMode === item.mode ? 'text-blue-600' : ''}`} />
